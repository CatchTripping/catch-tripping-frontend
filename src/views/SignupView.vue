...
watchEffect(() => {
  ...
  if (formData.value.confirmPassword) {
    ...
    errors.value.confirmPassword = isPasswordMatched.value
      ? ''
      : '비밀번호가 일치하지 않습니다.'
  } else {
...
watch(errors, newErrors => {
  if (newErrors.username === '이미 사용 중인 아이디입니다.')
    validationState.value.username = false
  if (newErrors.useremail === '이미 사용 중인 이메일입니다.')
    validationState.value.useremail = false
})
...
const register = async () => {
  ...
  try {
    ...
    alert(
      error.response?.data?.description || '회원가입 중 문제가 발생했습니다.',
    )
    if (error.response?.data?.description.includes('아이디'))
      validationState.value.username = false
    if (error.response?.data?.description.includes('이메일'))
      validationState.value.useremail = false
  }
}
...
const getInputClass = field => {
  if (field === 'password') {
    return validationState.value.password === false
      ? 'border-red-500'
  ...
  if (field === 'confirmPassword') {
    return validationState.value.confirmPassword === false
      ? 'border-red-500'
  ...
  >
    <div class="w-full max-w-md bg-white rounded-lg shadow-md p-8 space-y-4">
      ...
      <form class="space-y-4" @submit.prevent="register">
        ...
        <!-- Terms checkbox -->
        <div class="flex items-center space-x-2">
          <Checkbox id="terms" v-model:checked="formData.terms" />
          <Label for="terms" class="text-sm text-gray-600"
            >이용 약관 및 개인정보처리방침에 동의합니다.</Label
          >
        </div>
