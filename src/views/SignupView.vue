...
const isPasswordValid = computed(() => {
  return formData.value.password === '' ||
    (formData.value.password && PASSWORD_PATTERN.pattern.test(formData.value.password));
});
...
const isFormValid = computed(() => {
  const passwordValidation = formData.value.password && PASSWORD_PATTERN.pattern.test(formData.value.password);
  const passwordMatchValidation = formData.value.password === formData.value.confirmPassword;

  const conditions = {
    username: !!formData.value.username,
    useremail: !!formData.value.useremail,
    password: !!formData.value.password && passwordValidation,
    confirmPassword: !!formData.value.confirmPassword && passwordMatchValidation,
...
const updatePasswordError = () => {
  if (formData.value.password && !PASSWORD_PATTERN.pattern.test(formData.value.password)) {
...
const getInputClass = (field) => {
  if (field === 'password') {
    return formData.value.password && !isPasswordValid.value ? 'border-red-500' : '';
  }
  if (field === 'confirmPassword') {
    return formData.value.confirmPassword && !isPasswordMatched.value ? 'border-red-500' : '';
  }
